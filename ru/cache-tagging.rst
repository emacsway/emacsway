...
В существующей реализации статус метки (ACQUIRED или RELEASED) записывается в кэш состояния метки, вместе с идентификатором потока и временем начала транзакции.

На первый взгляд может показаться, что следовало бы использовать разноименные кэши для каждого статуса, ведь поток может освободить метку после того, как ее уже захватил параллельный поток, и сгенерировать кэш с неактуальными данными.

Но это только на первый взгляд.

- Во-первых, в веб-приложениях обычно вся обработка запроса клиента осуществляется в пределах одной транзакции. После этого коннект обычно либо закрывается, либо возвращается в пулл коннектов.
- Во-вторых, блокировка меток в данный момент реализована для уровня изоляции Repeatable read. А при таком уровне изоляции при попытке параллельными транзакциями изменить одину и ту же строку таблицы возникнет исключение "ERROR:  could not serialize access due to concurrent update".
